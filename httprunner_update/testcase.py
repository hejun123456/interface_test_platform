...
function_regexp = r"\$\{([\w_]+\s*\(.*?\))\}"
function_regexp_compile = re.compile(r"^([\w_]+)\s*\((.*?)\)$")
...
def parse_parameters(parameters, testset_path=None):
    ...
    for parameter in parameters:
        parameter_name, parameter_content = list(parameter.items())[0] if list(parameter.items()) else ('', [])
        # xxxxx
        # print("*"*50, "    ", parameter_content)
        ...
        if isinstance(parameter_content, list):
            ...
            # print("else here!!!!", type(parsed_parameter_content), "   ", parsed_parameter_content)
